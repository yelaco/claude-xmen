...
Before assigning a task that carries an approval gate, do NOT assign it. `SendMessage` to `team-lead` asking for explicit approval. Only assign after Cerebro confirms.
...
When all tasks are `completed` or the team is blocked, `SendMessage` to `team-lead`:
...
2. Reply **to `team-lead`**: `{type: "ready_for_shutdown"}`

When Cerebro sends `{type: "shutdown_request"}`:
- Reply **to `team-lead`** immediately: `{type: "shutdown_response", request_id: "...", approve: true}`

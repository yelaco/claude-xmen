...
| `/plan [task]` | Interview-based planning. Professor X asks clarifying questions, Beast checks for gaps, Emma Frost validates. Ends with a written plan in `.cerebro/plans/`. |
| `/start-work` | Execute the latest plan. Cyclops picks up from the last checkpoint if a session was interrupted. |
...
|---|---|---|
| **Professor X** | Strategic planner | `/plan` is called; needs to interview the user |
...
| **Cyclops** | Execution orchestrator | `/start-work` runs; never writes code directly |
...
Cerebro (CLAUDE.md)
 ...
 │
 ├── /plan ──────────► Professor X → Beast → Emma Frost
 │                              │
 │                              ▼
 │                    .cerebro/plans/{name}.md
 │
 └── /start-work ────► Cyclops (reads boulder.json, resumes or inits)
```

### Session Continuity

Every plan execution is tracked in `.cerebro/boulder.json`. If Claude Code stops mid-task, `/start-work` resumes exactly where it left off:
...
└── .cerebro/
    ...
    └── boulder.json                   # Execution state (created at /start-work)
```
...
```
/plan redesign the notification system to support real-time push
```
